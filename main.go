...
func main() {
	...
	err := server.ListenAndServeTLS("dist/3_application/certs/"+*domain+".cert.pem", "dist/3_application/private/"+*domain+".key.pem")
	if err != nil {
...
func createServerWithMTLS() *http.Server {
	...
	clientCACert, err := ioutil.ReadFile("dist/2_intermediate/certs/ca-chain.cert.pem")
	if err != nil {
		log.Fatal(err)
	}
